...
import { Leaf, LogOut, Moon, Settings, Sun } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-40 border-b bg-background/80 backdrop-blur">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4">
        <Link href={showAuthedNav ? "/dashboard" : "/"} className="flex items-center gap-2 font-semibold tracking-tight">
        ...
        <nav className="flex items-center gap-1">
          {showAuthedNav ? (
            <>
              ...
              {navLink("/species", "Species")}
              {navLink("/history", "History")}
              {navLink("/community", "Community")}
              {navLink("/help", "Help")}
              {isAdmin ? navLink("/admin", "Admin") : null}
            </>
          ) : null}
          <Button
            variant="ghost"
